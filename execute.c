...
/*   Updated: 2021/11/10 16:40:39 by yoyoo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parent_process(t_list **g_list, t_list **cmd_head,
		int *pipe_open, t_env **env)
{
	...
	if (pipe != 1 && (*cmd_head)->type != PIPE && (*cmd_head)->cmd_table)
		g_exit_status = (unsigned char)check_cmd(g_list, env,
				&(*cmd_head), my_envp);
	if (*pipe_open)
...
int	execute_builtin(t_list **cmd_head, t_list **g_list,
...
void	child_process(t_list **cmd_head, t_list **g_list,
		t_env **env, char **my_envp)
{
